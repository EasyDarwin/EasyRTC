...
int	EasyRTC_CreatePeer(EASYRTC_DEVICE_T* pDevice, EASYRTC_PEER_T* peer, EASYRTC_PEER_TYPE_E type, const char* offerSDP)
{
	...
	{
		// 先NAT穿透，不通再转为turn, 如需直接使用turn则置为EasyRTC_ICE_TRANSPORT_POLICY_RELAY
		pDevice->rtcConfiguration.iceTransportPolicy = EasyRTC_ICE_TRANSPORT_POLICY_ALL;// EasyRTC_ICE_TRANSPORT_POLICY_ALL;		
