...
int easyrtc_websocket_write(mbedtls_ssl_context* ssl, int sockfd, int opcode, char* data, int iLength)
{
	if (iLength < 126)
	{
		...
		return easyrtc_websocket_send_raw(ssl, sockfd, buffer, sizeof(ws_0m_header) + iLength);
	...
	else if (iLength < 65536)
	{
		...
		int ret = easyrtc_websocket_send_raw(ssl, sockfd, buffer, sizeof(ws_1m_header) + iLength);
		free(buffer);
		return ret;
	}
	else
	{
		...
		int ret = easyrtc_websocket_send_raw(ssl, sockfd, buffer, sizeof(ws_2m_header) + iLength);
		free(buffer);
		return ret;
	}
