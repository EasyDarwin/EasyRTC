...
static int net_init(WSS_CLIENT_T* ctx, int isSecure) {
	...
	if (isSecure) {
		...
		const char* pers = "easyrtc_client";
...
static int net_connect(WSS_CLIENT_T* ctx, const char* host, int port) {
	...
	if (ctx->isSecure) {
#ifdef USE_MBEDTLS
		if (mbedtls_net_connect(&ctx->net_ctx, host, port_str, MBEDTLS_NET_PROTO_TCP) != 0) {
			return -1;
		}
...
#endif
{
	...
	while (1) {
		...
		while (1) {
			...
			// === 复用你原有的 WebSocket 帧解析逻辑 ===
			// 注意：以下是你原来的帧处理代码（略作调整）
...
#endif
{
	...
	//__Print__(NULL, __FUNCTION__, __LINE__, false, "EasyRTC worker thread startup. [%d]\n", pThread->customId);

	const char httpRequest[] =
		//"GET /livegate/rtc?access_token=%s&auto_subscribe=1&sdk=cpp&version=1.0.1&protocol=12&adaptive_stream=1 HTTP/1.1\r\n"
		//"GET /rtc?access_token=%s&auto_subscribe=1&sdk=cpp&version=2.8.2&protocol=16 HTTP/1.1\r\n"

		//"GET /livegate/rtc?access_token=%s&auto_subscribe=1&sdk=js&version=2.13.1&protocol=16 HTTP/1.1\r\n"
		//"GET /rtc?access_token=%s&auto_subscribe=1&sdk=js&version=1.0.1&protocol=12&adaptive_stream=1 HTTP/1.1\r\n"
...
#ifdef _WIN32
		GUID* guid = (GUID*)tempKey; // GUID 长度刚好是16个字节
		CoCreateGuid(guid);
#else
		// 方法：从 /dev/urandom 读取 16 字节随机数据（等价于 GUID）
		FILE* urandom = fopen("/dev/urandom", "rb");
		if (urandom)
		{
			size_t bytesRead = fread(tempKey, 1, sizeof(tempKey), urandom);
			fclose(urandom);
	...
	while (1)
	{
		...
		{
			isDomainName = 0;		// 不为域名, 为IP
		}

		struct sockaddr_in servAddr = { 0 };
		servAddr.sin_family = AF_INET;
		servAddr.sin_port = htons(pWssClient->serverPort);

		while (1)
		{
			if (pThread->flag == THREAD_STATUS_EXIT)			break;

			//int nReceivedCount = 0; //收到的DNS解析包的个数
			//for (int i = 0; i < sizeof(NAMESERVERS) / sizeof(NAMESERVERS[0]); i++)
			{
				if (isDomainName)
				{
					char serverIP[64] = { 0 };
					resolve_domain_to_ip(pWssClient->serverAddr, serverIP, sizeof(serverIP));
					servAddr.sin_addr.s_addr = inet_addr(serverIP);
				}
				else
				{
					servAddr.sin_addr.s_addr = inet_addr(pWssClient->serverAddr);
				}

				if (servAddr.sin_addr.s_addr != 0) break;
			}
			if (servAddr.sin_addr.s_addr != 0) break;
		...
		//CallbackData(pDevice, NULL, EASYRTC_CALLBACK_TYPE_CONNECTING, 0, 0, NULL, 0, 0, 0);
		pWssClient->connectCallback(pWssClient->userptr, EASYRTC_WS_CONNECTING);

		pWssClient->sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		if (pWssClient->sockfd == INVALID_SOCKET) { Sleep(3000); continue; }

		rc = connect(pWssClient->sockfd, (struct sockaddr*)&servAddr, sizeof(struct sockaddr_in));
		if (rc < 0)
		{
			//CallbackData(pDevice, NULL, EASYRTC_CALLBACK_TYPE_CONNECT_FAIL, 0, 0, NULL, 0, 0, 0);
		...
		sprintf(httpHead, httpRequest, inet_ntoa(servAddr.sin_addr), pWssClient->serverPort, shakeKey);
		httpHeadLen = (int)strlen(httpHead);

		if (send(pWssClient->sockfd, httpHead, httpHeadLen, 0) != httpHeadLen)
		{
			closesocket(pWssClient->sockfd);
			pWssClient->sockfd = INVALID_SOCKET;
			Sleep(3000);
		...
		memset(recvbuf, 0, maxrecvlen);
		rc = recv(pWssClient->sockfd, recvbuf, maxrecvlen, 0);
		if (rc <= 0)
		{
			pWssClient->connectCallback(pWssClient->userptr, EASYRTC_WS_DISCONNECT);
		...
		if (strstr(recvbuf, "401 Unauthorized"))
		{
			// HTTP/1.1 401 Unauthorized

		}

		pdest = strstr(recvbuf, "\r\n\r\n"); // websocket的握手应答包接收完毕的标志是最后4个字节为"\r\n\r\n"
		...
		if (strncmp((const char*)recvbuf, "HTTP/1.1 101 Switching Protocols\r\n", 34) != 0)
		{
			//CallbackData(pDevice, NULL, EASYRTC_CALLBACK_TYPE_DISCONNECT, 0, 0, NULL, 0, 0, 0);
			pWssClient->connectCallback(pWssClient->userptr, EASYRTC_WS_DISCONNECT);
			closesocket(pWssClient->sockfd);
		...
		if ((strstr(recvbuf, "Sec-WebSocket-Accept: ") == NULL) &&
			(strstr(recvbuf, "Sec-Websocket-Accept: ") == NULL))
		{
			//CallbackData(pDevice, NULL, EASYRTC_CALLBACK_TYPE_DISCONNECT, 0, 0, NULL, 0, 0, 0);
			pWssClient->connectCallback(pWssClient->userptr, EASYRTC_WS_DISCONNECT);
			closesocket(pWssClient->sockfd);
		...
		pos = 0;
		easyrtc_socket_set_keepalive(pWssClient->sockfd);

		//pThis->CheckEasyRtcPeerConnection();

		time_t registerTime = 0;// time(NULL);
		pWssClient->registerStatus = 0;						// 置为未注册

		while (1)
		{
			if (pThread->flag == THREAD_STATUS_EXIT)			break;

			if (pWssClient->registerStatus == 0x00)
			{
				time_t tCurrent = time(NULL);
				if (tCurrent - registerTime > 5)
				{
					pWssClient->registerCallback(pWssClient->userptr);		// 发送注册包

					// 注册后由上层调用SetRegisterStatus设置注册状态
			...
			rc = select(pWssClient->sockfd + 1, &rfds, NULL, NULL, &timeout);
			if (rc == 0)
			{
				if (NULL != pWssClient->idleCallback)
				{
					pWssClient->idleCallback(pWssClient->userptr);
				}
			...
			if (!FD_ISSET(pWssClient->sockfd, &rfds)) continue;
			rc = recv(pWssClient->sockfd, recvbuf + pos, maxrecvlen - pos, 0);
			if (rc <= 0)
			{
				pWssClient->connectCallback(pWssClient->userptr, EASYRTC_WS_DISCONNECT);
				break;
		...
		NEXTLOOP1:
			...
			else if (wsheader->opcode == WS_OPCODE_PING)
			{
				...
				if (framesize > pos) continue; //没有接收完毕

				easyrtc_websocket_write(pWssClient->sockfd, WS_OPCODE_PONG, NULL, 0);

				if ((pos - framesize) > 0) memmove(recvbuf, recvbuf + framesize, pos - framesize);
				pos -= framesize;
				if (pos >= sizeof(ws_comm_header)) goto NEXTLOOP1;
			}
			else
			{
				if (wsheader->payloadlen < 126)
				{
					...
					if (ws0header->mask == 0)
					{
						...
						errCode = pWssClient->dataCallback(pWssClient->userptr, ws0header->payloaddata, payloadlen);
						//WebsocketDataHandler(pWssClient, ws0header->payloaddata, payloadlen, &errCode);
						if (errCode != 0)
						{
							break;
						}
					...
					else
					{
						...
						errCode = pWssClient->dataCallback(pWssClient->userptr, ws0mheader->payloaddata, payloadlen);
						//WebsocketDataHandler(pWssClient, ws0mheader->payloaddata, payloadlen, &errCode);
						if (errCode != 0)
						{
							break;
						}
				...
				{
					payloadlen = be16toh(ws1header->expayloadlen);
					if (ws1header->mask == 0) framesize = sizeof(ws_1_header) + payloadlen; else framesize = sizeof(ws_1m_header) + payloadlen;
					if (framesize > maxrecvlen)
					{
						int maxrecvlen2 = (framesize + 2048) / 1024 * 1024;
						char* recvbuf2 = (char*)malloc(maxrecvlen2);
					...
					if (ws1header->mask == 0)
					{
						ws1header->payloaddata[payloadlen] = '\0';
						errCode = pWssClient->dataCallback(pWssClient->userptr, ws1header->payloaddata, payloadlen);
						//WebsocketDataHandler(pWssClient, ws1header->payloaddata, payloadlen, &errCode);
						if (errCode != 0)
						{
							break;
						}
					...
					else
					{
						...
						errCode = pWssClient->dataCallback(pWssClient->userptr, ws1mheader->payloaddata, payloadlen);
						//WebsocketDataHandler(pWssClient, ws1mheader->payloaddata, payloadlen, &errCode);
						if (errCode != 0)
						{
							break;
						}
	...
	if (pWssClient->sockfd != INVALID_SOCKET)
	{
		closesocket(pWssClient->sockfd);
		pWssClient->sockfd = INVALID_SOCKET;
	}
	...
	//__Print__(NULL, __FUNCTION__, __LINE__, false, "EasyRTC worker thread shutdown. [%d]\n", pThread->customId);
...
int websocketGetVersion(char* version)
{
	...
	sprintf(version, "EasyRTC v1.0.%d.%02d%02d", year - 2000, month, day);
